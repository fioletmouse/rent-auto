...
export class RentService {
  ...
  async isAvailable(rentInput: RentInput): Promise<IRentOutput<boolean>> {
    ...
    if (this.utils.moreThanDayLimit(start, end)) {
      output.warnings.push(
        "You check period that is more than 30 days. Booking will be unavailable."
      );
    }
    if (this.utils.isWeekendDates(start, end)) {
      output.warnings.push(
        "Start or end date is a weekend day. Booking will be unavailable."
      );
    }
  ...
  async prelimCalc(rentInput: RentInput): Promise<IRentOutput<number>> {
    ...
    const res = await this.connection.query(
      'SELECT * FROM rate WHERE $1 BETWEEN start_date AND end_date ORDER BY "from" ASC',
      [start]
    );
  ...
  async book(rentInput: RentInput): Promise<IRentOutput<boolean>> {
    ...
    if (isAvailable.warnings.length > 0)
      throw new BadRequestException(isAvailable.warnings.join(" "));
    if (!isAvailable.result)
      throw new BadRequestException(
        "Car is unavailable. Please, try other dates."
      );
    ...
    try {
      const queryText =
        "INSERT INTO book_session(auto_id, start_date, end_date, total) VALUES($1, $2, $3, $4)";
      const res = await this.connection.singleCommand(queryText, [
        id,
        start,
        end,
        total,
